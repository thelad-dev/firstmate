...
`prime` is not a verified harness in this tree; [`docs/verification/prime-cursor-provider.md`](../../../docs/verification/prime-cursor-provider.md) owns its status and the standalone Cursor-as-Prime-model provider it refers to (a CLI-spawn provider extension, not a Firstmate harness).
